...
def contrastive_evaluate(val_loader, model, memory_bank):
    ...
    for batch in val_loader:
        images = batch['image'].cpu(non_blocking=True)
        target = batch['target'].cpu(non_blocking=True)
...
def get_predictions(p, dataloader, model, return_features=False):
    ...
    if return_features:
        ...
        features = torch.zeros((len(dataloader.sampler), ft_dim)).cpu()
...
def hungarian_evaluate(subhead_index, all_predictions, class_names=None, 
    ...
    targets = head['targets'].cpu()
    predictions = head['predictions'].cpu()
    probs = head['probabilities'].cpu()
    num_classes = torch.unique(targets).numel()
    num_elems = targets.size(0)

    match = _hungarian_match(predictions, targets, preds_k=num_classes, targets_k=num_classes)
    reordered_preds = torch.zeros(num_elems, dtype=predictions.dtype).cpu()
    ...
    f1 = metrics.f1_score(targets.cpu().numpy(), reordered_preds.cpu().numpy(), average='weighted')
    silhouette = metrics.silhouette_score(probs.cpu().numpy(), predictions.cpu().numpy())
    ...
    return {'ACC': acc, 'ARI': ari, 'NMI': nmi, 'ACC Top-5': top5, 'hungarian_match': match, 'FS' : f1, 'SIL':silhouette}
