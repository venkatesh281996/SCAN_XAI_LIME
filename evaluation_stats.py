...
parser.add_argument('--visualize_prototypes', action='store_true', help='Show the prototpye for each cluster')

args = parser.parse_args()

# Define evaluation functions (plot_silhouette_analysis, extrinsic_evaluation, intrinsic_evaluation)
...
def main():
    ...
    for n in number_of_candidates:
        ...
        neighborhood_indices = np.load("repository_eccv/cifar-20/pretext/top" + str(n) + "-val-neighbors.npy")
        ...
        clustering_stats = hungarian_evaluate(head, predictions, new_dataset.classes, compute_confusion_matrix=False)
        ...
        unique_values_count = [len(np.unique(predictions))]
        unique_values = [np.unique(predictions)]
        cluster_heads = [0]
        plot_silhouette_analysis(features, predictions, cluster_heads, unique_values_count, unique_values)
